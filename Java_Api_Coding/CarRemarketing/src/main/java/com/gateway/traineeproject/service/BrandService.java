...
public class BrandService {
	...
	public Brand update(Brand brandToBeUpdated) {
		...
		if (brandRepository.existsByName(brandToBeUpdated.getName())
				&& !brandRepository.existsByIdAndName(brandToBeUpdated.getId(), brandToBeUpdated.getName())) {
	...
	public void delete(Long id) {
		...
		if (modelRepository.existsByBrand(brand)) {
			throw new ConstraintViolationException("Brand can not be deleted as models are associated with this brand!",
					null, null);
		}
